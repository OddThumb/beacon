...
#' @export
get.available.detectors <- function() {
    check.installed(c("reticulate","tidyverse"))

    # functions from external libraries
    bind_rows <- dplyr::bind_rows
    Detector <- reticulate::import("pycbc.detector")$Detector
    get_available_detectors <- reticulate::import("pycbc.detector")$get_available_detectors
    ...
    det.df <- lapply(detector.list, function(idx) {
        ...
        name <- idx[[2]]
        d <- Detector(abv)
        data.frame("Detector Name"=name, "Abbreviation"=abv, "latitude"=d$latitude,"longitude"=d$longitude)
    })
    bind_rows(det.df)
}
...
#' @export
travel.times <- function(dets=c("H1","L1","V1","K1")) {
    check.installed("reticulate")
...
#' @export
relpass.time <- function(deltat, dets, ra, dec, t_gps, verbose=T) {
    check.installed("reticulate")
    ...
    if (length(dets) == 1) {
        ...
        dt_refs <- c()
        for (det in dets) {
            dt_refs[det] <- Detector(det)$time_delay_from_earth_center(ra, dec, t_gps)
        }
        ...
        for (i in 1:nrow(dets.grid)) {
            dref  <- dets.grid[ i, 1 ]
            dpass <- dets.grid[ i, 2 ]
            dt <- Detector( dpass )$time_delay_from_detector(Detector( dref ), ra, dec, t_gps)
            quota <- dt %/% deltat
            quota <- ifelse(quota < 0, quota+1, quota)
            dt_q <- quota*deltat
            dt_r <- (dt - dt_q) |> `names<-`(paste0("Ref: ",dref," / Pass: ",dpass))
        ...
        if (verbose) {
            message("> Time delays among detectors '",paste(dets,collapse = ', '),"'")
            print(det.df)
            message("> Because the time delay is not multiple of time resolution,")
            message("  following remainders are subtracted from preliminary time delay.")
            print(remainders)
        }
        
        return(list("dt_rel"=det.df, "dt_earth"=dt_refs, "remainder"=remainders))
    }
}
...
#' @export
get.antpatt <- function(det, ra, dec, pol, t_gps, online=F) {
    check.installed("reticulate")

    # If online=T, it takes quite long
    set.iers(online)

    if ( any(!hasArg(det), !hasArg(ra), !hasArg(dec), !hasArg(pol), !hasArg(t_gps)) )  {
        stop("All 4 arguments of 'ra', 'dec', 'pol', 't_gps' needs to be given for applying the antenna pattern. ")
    }

    Detector <- reticulate::import("pycbc.detector")$Detector
    d <- Detector(det)
    ap <- d$antenna_pattern(right_ascension = ra,
                            declination = dec,
                            polarization = pol,
                            t_gps = t_gps)
    names(ap) <- c("fp", "fc")
...
proj_wave <- function(hp, hc, det, ra, dec, pol, t_gps) {
    ...
    d <- Detector(det)
    ht <- d$project_wave(hp = hp.TS,
                         hc = hc.TS,
                         ra = ra,
                         dec = dec,
                         polarization = pol,
                         method = "lal")
    ts(reticulate::py_to_r(ht$data), 
...
#' @export
psd.noise <- function(det, duration, tstart, 
                      sampling.freq=4096, fl = 15, delta_f = 1/32,
                      seed = NULL) {
    if (det == "H1" | det == "L1") {
    ...
    NOISE.from_psd <- pycbc.noise$noise_from_psd(as.integer(tlen), 1/sampling.freq, PSD, seed)
    
    # Convert to ts
    noise.ts <- ts(NOISE.from_psd$data, start = tstart, frequency = sampling.freq)
